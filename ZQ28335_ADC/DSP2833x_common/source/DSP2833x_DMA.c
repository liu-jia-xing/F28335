...
void DMACH1AddrConfig(volatile Uint16 *DMA_Dest,volatile Uint16 *DMA_Source)
{
	...
	// 设置当前活动镜像源地址指针寄存器：SRC_ADDR_SHADOW
...
// 配置CH1的传输数据片（Burst）配置
// bsize：每片（bsize+1）字
// srcbstep：源地址增量（有符号数）
// desbstep：目标地址增量（有符号数）
void DMACH1BurstConfig(Uint16 bsize, int16 srcbstep, int16 desbstep)
{
	EALLOW;

	// Set up BURST registers:每个BURST片为（bsize+1）字
...
void DMACH1ModeConfig(Uint16 persel, Uint16 perinte, Uint16 oneshot, Uint16 cont, Uint16 synce, Uint16 syncsel, Uint16 ovrinte, Uint16 datasize, Uint16 chintmode, Uint16 chinte)
{
	...
	// PERINTSEL：Peripheral Interrupt Source Select Bits:
	// PERINTSEL:存放接入DMA CH1传输时隙的中断源编号，共32种，
	DmaRegs.CH1.MODE.bit.PERINTSEL = persel;	    // Passed DMA channel as peripheral interrupt source
	// PERINTE:Peripheral Interrupt Trigger DMA Enable Bit:
	// PERINTE=1：enable，0：disable
	DmaRegs.CH1.MODE.bit.PERINTE = perinte;       	// Peripheral interrupt enable
	// ONESHOT： One Shot Mode Bit:
	// 0：一个事件触发一个时隙传输，1：一个事件触发时隙连续传输
	DmaRegs.CH1.MODE.bit.ONESHOT = oneshot;       	// Oneshot enable

	// CONTINUOUS:Continuous Mode Bit:
	// 0:the DMA stops and clears the RUNSTS bit to 0.
	// 1:DMA re-initializes when TRANSFER_COUNT is zero and waits for the next ...
	...
	// Channel Interrupt Generation Mode Bit:
	...
	// 0：disables the respective DMA channel interrupt to the CPU (via the PIE).
	// 1：enables the respective DMA channel interrupt to the CPU (via the PIE).
