//配置EQEP1和EQEP2引脚：
//EQEP1引脚：GPIO20 (EQEP1A)、GPIO21 (EQEP1B)、GPIO22 (EQEP1S)、GPIO23 (EQEP1I)
//                           或GPIO50~53
//EQEP2引脚：GPIO24 (EQEP2A)、GPIO25 (EQEP2B)、GPIO26 (EQEP2S)、GPIO27 (EQEP2I)
//以上引脚特性：均 不上拉，不同步到SYSCLKOUT，
...
// 光电编码器输出信号：QEPA
//              QEPB(顺转时，落后QEPA 90 degreee)
//              QEPI(脉冲索引信号)

// eQEP模块输入信号：QEPA
//              QEPB(顺转时，落后QEPA 90 degreee)
//              QEPI(脉冲索引信号)
//                QEPS(电机的限位开关信号，用于表示电机已经到达指定位置，输入到DSP，用于锁存QEP模块内部的计数器的值)
